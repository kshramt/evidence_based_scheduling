...
async fn main() {
    ...
    // Sentinel: Security headers added to mitigate XSS (CSP), clickjacking (X-Frame-Options), and MIME-sniffing (X-Content-Type-Options).
    // Since this is an API, strict default-src 'none' is safe.
    let app = app
