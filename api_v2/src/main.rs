...
fn app(state: std::sync::Arc<AppState>) -> axum::Router {
...
mod tests {
    ...
    async fn test_security_headers() {
        // Use a dummy connection string to prevent CI failures.
        let pool = sqlx::postgres::PgPoolOptions::new()
            .connect_lazy("postgres://invalid:invalid@localhost/invalid")
            .unwrap();
        let state = std::sync::Arc::new(AppState::new(0, pool));

        // Target a non-existent route to test middleware execution without database queries
        let app = app(state);
