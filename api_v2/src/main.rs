...
mod tests {
    ...
    async fn test_security_headers() {
        // Create dummy app state with a lazy connection pool that won't connect unless queried
        ...
        // Create a mock request to a non-existent route to test middleware execution
        let request = Request::builder()
            .uri("/api/v2/not-found")
            .body(Body::empty())
            .unwrap();

        // Send the request through the middleware stack
