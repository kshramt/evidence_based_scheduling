...
// ⚡ Bolt: React.memo prevents unnecessary O(N) re-renders in tree/list lists
...
// ⚡ Bolt: React.memo prevents unnecessary O(N) re-renders in tree/list lists
const MobileEntryButtons = React.memo((props: { node_id: types.TNodeId }) => {
  const leaf_estimates_sum = utils.assertV(
    useSelector(
      (state) => state.swapped_caches.leaf_estimates_sum?.[props.node_id],
    ),
...
MobileEntryButtons.displayName = "MobileEntryButtons";
