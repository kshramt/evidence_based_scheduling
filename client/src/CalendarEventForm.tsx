...
const CalendarEventFormImpl = (props: {
  event?: undefined | types.TEvent;
  onSubmit: (event: types.TEvent) => void;
}) => {
  const today = { f: Math.floor(times.getFloatingNow().f / DAY) * DAY };
  const tomorrow = { f: today.f + DAY };
  ...
  const handleLimitTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
