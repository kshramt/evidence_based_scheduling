...
test("dnd_move", () => {
  expect(T.dnd_move([1, 2, 3], 0, 1)).toEqual([2, 1, 3]);
  expect(T.dnd_move([1, 2, 3], 0, 2)).toEqual([2, 3, 1]);
  expect(T.dnd_move([1, 2, 3, 4, 5], 1, 3)).toEqual([1, 3, 4, 2, 5]);
  expect(T.dnd_move([0, 1, 2, 3, 4], 3, 1)).toEqual([0, 3, 1, 2, 4]);
  expect(T.dnd_move([0, 1, 2, 3, 4], 3, 1)).toEqual([0, 3, 1, 2, 4]);
});
...
test("cumsum", () => {
  expect(T.cumsum([1, 2, 3])).toEqual([0, 1, 3, 6]);
});

test("Multinomial", () => {
  {
    const vals = [9, 8, 7];
    const rng = new T.Multinomial([1, 2, 3]);
    const ns = [];
  ...
  {
    const vals = [9, 8];
    const rng = new T.Multinomial([1, 1]);
    const ns = [];
  ...
  }
  {
    ...
    const p = 100 / w_total;
    const rng = new T.Multinomial(ws);
    const total = 100000;
    let count = 0;
